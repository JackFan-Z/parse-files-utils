...
function initialize(config) {
  ...
  questions(config).then(function (answers) {
    ...
    // Sort by updatedAt
    console.log('Sorting files by updatedAt ...');
    objects.sort(function( f1, f2) {
      return f1.updatedAt - f2.updatedAt;
    })
...
function getObjectsWithFilesFromSchema(schema) {
  ...
  console.log('schema.className ' + schema.className + '. ' + schema.fields);
